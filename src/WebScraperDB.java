...
public class WebScraperDB
{
    ...
    public WebScraperDB(String dbUrl) throws SQLException{
        try {
    ...
    public void refreshDb( ) {
        try {
            Statement statement = connection.createStatement();
            ResultSet rs = statement.executeQuery("DELETE FROM annunci WHERE 1=1");
        } catch (SQLException e) {
            System.err.println("Unable to delete table from database");
        }
        scrape();
    }
    ...
    private void autoscout() {
        String url = "https://www.autoscout24.it/lst";
        String queryParams = "?offer=U&page=";
        Set<String> brands = new HashSet<String>();
        try {
            System.err.println("Searching in " + url);
            for(int i = 1; i < PAGE_LIMIT; i++) {
                Document doc = Jsoup.connect(url + queryParams + i).get();
                ...
                for(var article : articles) {
                    brands.add(article.attr("data-make").toLowerCase());
                }
            }
...
//                            System.err.println("Constraint violation, not inserting"); //troppa roba
                            System.err.print("");
                        }
                    }
                }
            ...
            System.err.println("Http error url = " + url);
        } catch(Exception e) {
            e.printStackTrace();
        }
    }
